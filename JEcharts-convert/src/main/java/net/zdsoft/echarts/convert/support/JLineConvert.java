...
public class JLineConvert extends JDataConvertRoot {
    ...
    public void convert(JData data, Option option, JDataConvertChain chain) {
        ...
        for (JData.Entry entry : data.getEntryList()) {
            Line line = lines.computeIfAbsent(entry.getName(), name -> new Line().option(option));
            line.data(line.create().name(entry.getX()).value(entry.getY()).parent(line)).stack(entry.getStack());
